module RapidResources
  class ResourceResolver
    ...
    def page(action, current_user, page_class: nil, url_helpers: nil)
      unless page_class
        @page_class ||= Object.const_get("#{@controller_path.singularize.camelize}Page")
        page_class = @page_class
      end
      page_class.new(current_user, name: @controller_path, model_class: self.model_class, action: action, url_helpers: url_helpers)
    end
